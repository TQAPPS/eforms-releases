import os
import sys
import subprocess
import base64
import re

FALLBACK_KEYSTORE_B64 = ''

def is_valid_keystore(path, password, alias):
    if not os.path.exists(path) or os.path.getsize(path) == 0:
        return False
    try:
        res = subprocess.run(
            ["keytool", "-list", "-keystore", path, "-storepass", password, "-alias", alias],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
        return res.returncode == 0
    except Exception as e:
        print(f"Warning running keytool: {e}")
        return False

def setup():
    os.makedirs("android/app", exist_ok=True)
    keystore_path = "android/app/my-release-key.jks"
    
    store_password = os.environ.get("STORE_PASSWORD", "Nova0566#").strip() or "Nova0566#"
    key_password = os.environ.get("KEY_PASSWORD", "Nova0566#").strip() or "Nova0566#"
    key_alias = os.environ.get("KEY_ALIAS", "my-key-alias").strip() or "my-key-alias"
    
    keystore_b64 = os.environ.get("KEYSTORE_BASE64", "").strip()
    restored_from_secret = False
    
    if keystore_b64:
        try:
            clean = re.sub(r'-----.*?-----|\s+', '', keystore_b64)
            missing_padding = len(clean) % 4
            if missing_padding:
                clean += '=' * (4 - missing_padding)
            decoded_bytes = base64.b64decode(clean)
            temp_test_path = "android/app/temp-test-keystore.jks"
            with open(temp_test_path, "wb") as f:
                f.write(decoded_bytes)
            
            if is_valid_keystore(temp_test_path, store_password, key_alias):
                with open(keystore_path, "wb") as f:
                    f.write(decoded_bytes)
                print(f"KEYSTORE_BASE64 secret is valid ({len(decoded_bytes)} bytes). Restored successfully.")
                restored_from_secret = True
            else:
                print(f"Warning: KEYSTORE_BASE64 secret ({len(decoded_bytes)} bytes) is corrupted or password/alias mismatch. Will use verified embedded keystore.")
            
            if os.path.exists(temp_test_path):
                os.remove(temp_test_path)
        except Exception as e:
            print(f"Warning: Could not decode KEYSTORE_BASE64 secret ({e}).")

    if not restored_from_secret:
        # Write the exact verified embedded keystore
        with open(keystore_path, "wb") as f:
            f.write(base64.b64decode(FALLBACK_KEYSTORE_B64))
        print(f"Verified embedded keystore restored ({os.path.getsize(keystore_path)} bytes).")
        
    # Verify final keystore
    if is_valid_keystore(keystore_path, store_password, key_alias):
        print(f"Keystore verification PASSED for alias '{key_alias}'.")
    else:
        print("ERROR: Final keystore verification failed!")
        sys.exit(1)

    key_props_path = "android/key.properties"
    with open(key_props_path, "w", encoding="utf-8") as f:
        f.write(f"storePassword={store_password}\n")
        f.write(f"keyPassword={key_password}\n")
        f.write(f"keyAlias={key_alias}\n")
        f.write("storeFile=my-release-key.jks\n")
    print("Written android/key.properties successfully.")

if __name__ == "__main__":
    setup()
